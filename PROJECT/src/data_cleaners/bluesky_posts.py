...
def main(input_path: str, output_path: str):
    ...
    # If like_count, repost_count, reply_count or quote_count are not present, set them to 0
    df = df.fillna({"like_count": 0, "repost_count": 0, "reply_count": 0, "quote_count": 0})

    # Cast created_at to timestamp
    df = df.withColumn("created_at", df["created_at"].cast("timestamp"))

    # Cast like_count, repost_count, reply_count and quote_count to int
